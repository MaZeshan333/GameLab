...
class CloudWordBattle:
        ...
        try:
            ...
            if response.status_code == 200 and response.json():
                ...
                self.leaderboard_data = sorted(
                    raw_list, key=lambda x: x["score"], reverse=True
                )[:20]
    ...
    def check_and_remove(self, word):
        ...
        if found:
            ...
            for c in range(GRID_SIZE):
                letters = [
                    self.grid[r][c] for r in range(GRID_SIZE) if self.grid[r][c] != ""
                ]
    ...
    def draw(self):
        ...
        if self.state == "START":
            self.screen.blit(
                self.main_font.render("WORD STACK CLOUD", True, BLUE),
                (SCREEN_WIDTH // 2 - 145, 150),
            )
            self.screen.blit(
                self.ui_font.render(f"NAME: {self.player_name}", True, BLACK),
                (SCREEN_WIDTH // 2 - 80, 220),
            )
            self.screen.blit(
                self.small_font.render("Type name then press SPACE", True, GRAY),
                (SCREEN_WIDTH // 2 - 105, 280),
            )

        elif self.state == "PLAYING":
            t_color = RED if self.time_left < 10 else BLACK
            self.screen.blit(
                self.ui_font.render(f"TIME: {self.time_left}s", True, t_color), (40, 20)
            )
            self.screen.blit(
                self.ui_font.render(f"SCORE: {self.score}", True, GREEN), (280, 20)
            )
            self.screen.blit(
                self.small_font.render(self.status_msg, True, GRAY), (40, 60)
            )
            ...
            self.screen.blit(
                self.ui_font.render("NEXT", True, BLACK), (p_x + 12, GRID_OFFSET_Y - 25)
            )
            self.screen.blit(
                self.main_font.render(self.next_letter, True, BLUE),
                (p_x + 25, GRID_OFFSET_Y + 25),
            )
            ...
            for r in range(GRID_SIZE):
                for c in range(GRID_SIZE):
                    ...
                    if self.grid[r][c]:
                        pygame.draw.rect(
                            self.screen, BLUE, rect.inflate(-6, -6), border_radius=10
                        )
            ...
            # Input
            i_rect = pygame.Rect(
                GRID_OFFSET_X, SCREEN_HEIGHT - 60, (GRID_SIZE * CELL_SIZE), 40
            )
            pygame.draw.rect(self.screen, BLACK, i_rect, 2)
            self.screen.blit(
                self.ui_font.render(f"INPUT: {self.input_text}", True, BLACK),
                (GRID_OFFSET_X + 10, SCREEN_HEIGHT - 55),
            )

        elif self.state == "FINISH":
            self.screen.blit(
                self.main_font.render("TIME UP!", True, RED),
                (SCREEN_WIDTH // 2 - 65, 30),
            )
            self.screen.blit(
                self.ui_font.render(f"YOUR SCORE: {self.score}", True, BLACK),
                (SCREEN_WIDTH // 2 - 75, 75),
            )

            # Leaderboard
            self.screen.blit(
                self.ui_font.render("-- TOP 20 GLOBAL --", True, GOLD),
                (SCREEN_WIDTH // 2 - 95, 120),
            )
            for i, entry in enumerate(self.leaderboard_data):
                ...
                self.screen.blit(
                    self.small_font.render(txt, True, color),
                    (SCREEN_WIDTH // 2 - 80, 150 + i * 20),
                )

            if self.uploading:
                self.screen.blit(
                    self.small_font.render("Syncing...", True, GRAY),
                    (20, SCREEN_HEIGHT - 30),
                )
            else:
                self.screen.blit(
                    self.small_font.render("Press 'R' to Restart", True, BLUE),
                    (20, SCREEN_HEIGHT - 30),
                )
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if self.state == "START":
                        ...
                        self.state = "START"
                        threading.Thread(
                            target=self.fetch_leaderboard, daemon=True
                        ).start()
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and self.state == "PLAYING":
                    ...
                    if GRID_OFFSET_X <= mx <= GRID_OFFSET_X + GRID_SIZE * CELL_SIZE:
                        ...
                        if not self.drop_letter(col):
                            self.state = "FINISH"
                            threading.Thread(
                                target=self.upload_score, daemon=True
                            ).start()
